...
def render_kotak_vertikal(key_id, class_node="node-staf", label_def="STAF"):
    ...
    with st.container():
        st.markdown(f'<div class="box-bagan-vertikal {class_node}"><p class="text-jabatan">{staf["jabatan"] if staf["jabatan"] else label_def}</p>', unsafe_allow_html=True)
        if staf["foto"] is not None: st.image(staf["foto"], width=120)
        else: st.markdown('<div class="placeholder-foto">📷<br>(Upload Foto)</div>', unsafe_allow_html=True)
        st.markdown(f'<p class="text-nama">{staf["nama"] if staf["nama"] else "Belum Diisi"}</p></div>', unsafe_allow_html=True)
        
        if st.session_state.is_admin_logged_in:
            with st.popover("⚙️ Edit", use_container_width=True, key=f"pop_{key_id}"):
                ...
                f_upload = st.file_uploader("Foto:", type=["jpg","jpeg","png"], key=f"f_{key_id}")
                if f_upload: st.session_state.struktur_data[key_id]["foto"] = f_upload.read()
                if st.button("Simpan 💾", key=f"btn_{key_id}", use_container_width=True): st.rerun()
...
with st.sidebar:
    ...
    if menu == "PROFIL":
        st.markdown("---")
        sub_menu = st.radio("SUB PROFIL:", ["PROFIL LEMBAGA", "STRUKTUR ORGANISASI"])
    elif menu == "PROGRAM":
        st.markdown("---")
        sub_menu = st.radio("SUB PROGRAM:", ["JURUSAN", "SYARAT", "BIAYA"])
    elif menu == "ABSENSI":
        st.markdown("---")
        sub_menu = st.radio("SUB ABSENSI:", ["ABSENSI GURU", "ABSENSI SISWA"])
...
if menu == "PROFIL":
    if sub_menu == "PROFIL LEMBAGA":
    ...
    elif sub_menu == "STRUKTUR ORGANISASI":
        ...
        with c_vd1: render_kotak_vertikal("mid_1", "node-wadir", "WAKIL DIREKTUR I")
        with c_vd2: render_kotak_vertikal("mid_2", "node-wadir", "WAKIL DIREKTUR II")
        st.write("<div style='text-align:center; color:#CBD5E1; font-weight:bold; margin:-5px 0;'>▼</div>", unsafe_allow_html=True)
        c_st1, c_st2, c_st3, c_st4 = st.columns(4)
        with c_st1: render_kotak_vertikal("bot_1", "node-staf", "STAF F&B PRODUCT")
